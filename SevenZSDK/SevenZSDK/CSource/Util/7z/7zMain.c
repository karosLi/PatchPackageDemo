...
static WRes MyCreateCharDir(const char *name)
{
  WRes res;
  res =
  #ifdef _WIN32
...
static WRes OutFile_OpenCharStr(CSzFile *p, const char *name)
{
  WRes res;
  res = OutFile_Open(p, name);
  return res;
}

static WRes Utf16_To_CharStr(const UInt16 *s, char **outChar) {
    CBuf buf;
    WRes res;
    Buf_Init(&buf);
    res = Utf16_To_Char(&buf, s MY_FILE_CODE_PAGE_PARAM);
    if (res == SZ_OK)
        *outChar = (char *)buf.data;
    Buf_Free(&buf, &g_Alloc);
    return res;
}
...
int MY_CDECL sevenZ_extract(const char *archivePath, char **error)
{
  ISzAlloc allocImp;
  ISzAlloc allocTempImp;
